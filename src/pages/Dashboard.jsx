...
function Dashboard() {
  const { patients, selectedPatientName, loading, error, dashboardData } = usePatientData()

  if (loading) {
    return <main className="grid min-h-screen place-items-center bg-[#f6f7f8]">Loading dashboard...</main>
  }
  ...
  return (
    <main className="min-h-screen w-full bg-[#f6f7f8] p-3 sm:p-4 lg:h-screen lg:overflow-hidden lg:p-5">
      <div className="mx-auto flex min-h-full w-full max-w-[1600px] flex-col gap-3 lg:h-full lg:gap-4 lg:overflow-hidden">
        ...
        <section className="grid min-h-0 flex-1 grid-cols-1 gap-3 lg:gap-4 xl:grid-cols-[320px_minmax(0,1fr)_320px]">
          <div className="min-h-0 xl:block">
            <Sidebar patients={patients} selectedPatientName={selectedPatientName} />
          </div>

          <div className="dashboard-scrollbar min-h-0 space-y-3 lg:space-y-4 lg:overflow-y-auto lg:pr-1">
            <DiagnosisChart chartData={dashboardData.chartData} bpSummary={dashboardData.bpSummary} />
